...
class addon_updater_updated_successful(bpy.types.Operator):
	...
	def draw(self, context):
		...
		if self.error != "":
			col = layout.column()
			col.scale_y = 0.7
			col.label("Need to Login First", icon="ERROR")
			col.label(updater.error_msg, icon="BLANK1")
			rw = col.row()
			rw.scale_y = 2
			rw.operator("d3tool.services_get_token", text="Login")
...
def update_notice_box_ui(self, context):
	...
	if updater.manual_only==False:
		...
		col.operator("wm.url_open", text="Open website to download new version").url = updater.website
...
def update_settings_ui(self, context, element=None):
	...
	#			"Check for D3Splint Update" ).url=updater.website		
		col.operator(addon_updater_check_now.bl_idname,
					 "Check for D3Dual Update")
...
def register(bl_info):
	...
	updater.private_token = "G8RxB2AHCWwj1ghgJTzK"

	# choose your own username, must match website (not needed for GitLab)
	updater.user = "d3splint_updater"

	# choose your own repository, must match git name
	updater.repo = "7605197"

	#updater.addon = # define at top of module, MUST be done first

	# Website for manual addon download, optional but recommended to set
	updater.website = "https://d3tool.com/my-account/downloads/"
	...
	updater.version_min_update = (0,4,8)
