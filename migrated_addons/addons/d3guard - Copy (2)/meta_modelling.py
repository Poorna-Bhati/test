...
@author: Patrick
interesting reads  https://stackoverflow.com/questions/59825/how-to-retrieve-an-element-from-a-set-without-removing-it
'''
import bpy
...
class D3SPLINT_OT_draw_meta_curve(bpy.types.Operator):
    ...
    def invoke(self,context, event):
        ...
        #tracking.trackUsage("D3DUAL:MetaCurveDraw", None)
        return {'RUNNING_MODAL'}
...
class D3SPLINT_OT_surgical_bite_positioner(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        #tracking.trackUsage("D3DUAL:SurgicalBitePositioner",None)
        ...
        #get world path of the maxillary curve  #TODO evaluated bezier points directly w/o a dumb to_mesh
        ...
        #get world path of the mandibular curve #TODO evaluated bezier points directly w/o a dumb to_mesh
        ...
        #min_x_mand =  min(vs_even_mand, key = lambda x: x[0])
        ...
        for i in range(1,len(vs_even_max)-1):
            ...
            #use maxilary curve for estimating
            v0_0 = vs_even_max[i]
...
class D3SPLINT_OT_anterior_deprogrammer_edit(bpy.types.Operator):
    ...
    hold_update = BoolProperty(default = True, description = 'Pause live update')
...
class D3SPLINT_OT_splint_join_depro_to_shell(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        #tracking.trackUsage("D3DUAL:JoinDeprogrammer",None)
...
class D3SPLINT_OT_splint_join_meta_to_shell(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        #tracking.trackUsage("D3DUAL:JoinVirtualWax",None)
...
class D3SPLINT_OT_blockout_splint_shell(bpy.types.Operator):
    ...
    def execute(self, context):
        #tracking.trackUsage("D3DUAL:BlockoutSplintConcavities",None)
        ...
        context.scene.update()
        finish = time.time()
        print('Blocked out shell in %f seconds' % (finish-start))
        return {'FINISHED'}        
...
class D3SPLINT_OT_blockout_trimmed_model2(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        ob1 = bpy.data.objects.get('Refractory Scaffold')
        ...
        #tracking.trackUsage("D3DUAL:RemoveUndercuts", (str(self.radius)[0:4], str(total_meta_time)[0:4]), background = True)
        return {'FINISHED'}
...
class D3SPLINT_OT_refractory_model(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not Trim:
            self.report({'ERROR'}, 'Must mark margin first')
            return {'CANCELLED'}
        ...
        print('TOTAL FREAKING TIME %f' % (global_total_finish - global_total_start))  
        ...
        #tracking.trackUsage("D3DUAL:RemoveUndercuts", (str(self.b_radius)[0:4], str(self.b_radius)[0:4], str(total_meta_time)[0:4]), background = True)
        return {'FINISHED'}
...
class D3SPLINT_OT_drill_compensation(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        new_ob.hide = False
        splint.refractory_model = True
        #tracking.trackUsage("D3DUAL:DrillCompensation", (str(self.b_radius)[0:4], str(self.b_radius)[0:4], str(total_meta_time)[0:4]), background = True)
        '''
...
class D3SPLINT_OT_sculpt_concavities(bpy.types.Operator):
    ...
    def execute(self, context):
        #tracking.trackUsage("D3DUAL:SculptSplintConcavities",None)
        ...
        for ed in bme.edges:
            if len(ed.link_faces) != 2: continue
