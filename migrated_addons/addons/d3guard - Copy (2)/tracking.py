...
from validation import get_cloud_key
...
def get_sha_license():
    ...
    key = get_cloud_key()
    SHA = sha1(key.encode())
    sha_key = SHA.hexdigest()
    final_key = sha_key.upper()
...
class Singleton_tracking(object):
    ...
    def __init__(self):
        ...
        self._bg_thread = []
        self._version = ""
        self._sha_license = get_sha_license()
    ...
    def initialize(self, track_install):
        ...
        self._ip_location_info = {"city":city, "country":country, "loc":str(loc)}
    ...
    def set_tracker_json(self):
        ...
        #if there is a file already, then read it
        if os.path.isfile(self._tracker_json)==True:
            print('self._tracker_json is file! so we are alreday installed')
            with open(self._tracker_json) as data_file:
                self.json = json.load(data_file)
            if self._verbose:print("Reading in json settings from optimization file")
           
        
        else: #not os.path.isfile(self._tracker_json):

            #print(self._tracker_json)
            #print(self.json)
            #print(os.path.isfile(self._tracker_json), print("status" in self.json))
            # set data structure
...
def register(bl_info):
    ...
    Tracker.initialize_bg_and_track_install(
        appurl = "https://d3dual-database.firebaseio.com/",
