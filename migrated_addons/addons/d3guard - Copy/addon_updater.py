...
from update_authenticate.update import download_nonthreaded
...
#some extra specal imports
from .update_authenticate.update import services_get_newest_ver, get_addon_local_version, services_get_saved_token, check_token, force_get_signed_url
# -----------------------------------------------------------------------------
...
class Singleton_updater(object):
    ...
    def stage_repository(self, url):
        ...
        self._source_zip = os.path.join(local,"code_package.zip")
        
        if self._verbose: print("Starting download update zip")
        try:
            #need way to insert token
            token = services_get_saved_token()
            if not check_token(token):
    
                self._error = "Bad Token, Re-authenticate?"
                self._error_msg = "Error: {}".format('Bad token')
                if self._verbose:
                    print("Error retrieving download, bad link?")
                    print("Error: {}".format('Bad Toekn'))
                return False
            
            signed_url = force_get_signed_url(token)
            
            download_nonthreaded(signed_url,"code_package.zip",local)
    ...
    def check_for_update(self, now=False):
        ...
        cloud_version = services_get_newest_ver()
        local_version = self.version_tuple_from_text(get_addon_local_version())
        
        new_version = self.version_tuple_from_text(cloud_version)  #self._tag_latest
        
        
        print('CHECKING FOR UDPATES')
        print(cloud_version, local_version, new_version)
        
        if new_version > local_version:  #self.current_version?
            link = 'www.google.com'
            self._update_ready = True
