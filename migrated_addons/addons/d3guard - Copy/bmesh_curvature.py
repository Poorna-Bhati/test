...
def calculate_plane(locs, itermax = 500, debug = False):  #TODO, consolidat utilities
    '''
    args: 
    vertex_locs - a list of type Vector
    return:
    CoM and normal of best fit plane
    '''
    if debug:
        start = time.time()
        n_verts = len(locs)
...
def concave(bmvert, lam):
    ...
    if vdiff.dot(no) > lam:
        #bmvert.select = True  #used for vferification
        return 1
    else:
        return 0
...
class CuspWaterDroplet(object):
    ...
    def hfunc(self,bvert):
        ...
        H = (1 - self.alpha) * K - self.alpha * vz  #perhaps height and curvature need to be normalized somehow?  Yes, normalized over max_height and max curvature
