...
class BracketDataManager(object):
    ...
    def place_bracket(self,context,x,y, normal = False, lock_x = False):
        self.grab_initiate()
        res = self.grab_mouse_move(context, x, y, normal = normal, lock_x = lock_x)
        self.grab_confirm()
        return res
    ...
    def grab_mouse_move(self,context,x,y, normal = False, lock_x = False):
        ...
        if self.snap_type == 'SCENE':
            ...
            omx = self.bracket_obj.matrix_world
        ...
        if not hit:
            ...
            if normal:
                ...
                if lock_x:
                    #print("LOCK X")
                    w_x = omx.to_quaternion() * Vector((1,0,0))
                    ob_Y = ob_Z.cross(w_x)
    ...
    def spin_event(self, event, shift, step = 2.5):
        ...
        if shift:
            ang = .2 * step * math.pi/180
        else:
            ang = step *math.pi/180
    ...
    def rotate_event(self, event, shift, step = 2.5):
