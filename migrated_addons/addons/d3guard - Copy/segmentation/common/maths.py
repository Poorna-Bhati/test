...
def intersect_path_plane(v_path, pt, no, r_max = 20, epsilon = .00001):
    min_dist = float('inf')
    min_cross = None
    coords = {}
    for i in range(0, len(v_path)-1):
        
        
        a, b = v_path[i], v_path[i+1]
        if i not in coords: coords[i] = no.dot(a-pt)
        if (i+1) not in coords: coords[i+1] = no.dot(b-pt)

        s0,s1 = coords[i],coords[i+1]
        if s0 > epsilon and s1 > epsilon: continue
        if s0 < -epsilon and s1 < -epsilon: continue

        cross = intersect_line_plane(a, b, pt, no)
