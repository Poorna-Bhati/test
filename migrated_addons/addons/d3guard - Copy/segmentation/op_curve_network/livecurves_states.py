...
from segmentation.op_curve_network.livecurves_datastructure import InputPoint, SplineSegment, CurveNode
...
class Livecurves_States():
    ...
    def spline_tweak(self):
        ...
        if self.tweak_moving and self.actions.mousemove:
            ...
            if self.net_ui_context.target:
                self.net_ui_context.update(p2d)
                self.grabber.move_grab_point(self.context, p2d)
                if self.snap_tessellation:
                    self.snap_node(self.net_ui_context.selected)
            
            else:
                self.grabber.move_grab_point_2D(self.context, p2d)
    ...
    def spline_add_point_can_enter(self):
        ...
        if self.net_ui_context.hovered_mesh == {}: return False 
        return True
    ...
    def spline_insert_point_can_enter(self):
        # make sure mouse is over source if source is required
        if self.net_ui_context.target and self.net_ui_context.hovered_mesh == {}: return False 
        #and over a spline segment
        hn = self.net_ui_context.hovered_near
        if not hn and self.net_ui_context.target != None: return False
        if hn[0] != 'EDGE': return False
        #now check we are over top of appropriate segment
        #seg = hn[1]
        #n0, n1 = seg.n0, seg.n1
