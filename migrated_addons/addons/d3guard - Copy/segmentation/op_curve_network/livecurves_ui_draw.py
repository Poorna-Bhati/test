...
from segmentation.op_curve_network.livecurves_datastructure import InputPoint, CurveNode, SplineSegment
...
orange2_trans    = (1.0, 0.65, 0.2, 0.5)
...
purple_trans      = (0.8, 0.1, 0.8, 0.5)
pink        = (1.0, 0.8, 0.8, 1.0)

preview_line_clr = (0.4, 1.0, 0.4, 0.7)
preview_line_wdth = 5


snap_radius = 32
connect_line_width = 3
select_radius = 15
...
class Livecurves_UI_Draw():
    ...
    def draw_postview(self):
        ...
        if self.net_ui_context.hovered_near[0] in {'NON_MAN_ED', 'NON_MAN_VERT'}:
            ...
            self.draw_circle(loc, select_radius, .7, green_trans, clear)

        #INSERT POINT HINT
        if self.net_ui_context.hovered_near[0] in {'EDGE'}:
            # draw insertion circle in 3d
            if self.net_ui_context.hovered_mesh != {}:
        ...
        if self.net_ui_context.snap_element:
            ...
            self.draw_circle(loc, snap_radius, select_radius/snap_radius, green_trans, clear)
    ...
    def draw_2D_stuff(self):
        ...
        #        if ip.is_endpoint:
        #            common_drawing.draw_3d_points(context,[ip.world_loc], 10, yellow)
        ...
        if self._state == 'spline' and self.net_ui_context.snap_element and self.net_ui_context.selected:
            ...
            #draw connect hint
            if psnap != psel and psnap.is_endpoint and psel.is_endpoint:
                pn = psnap.world_loc
                ps = psel.world_loc
        ...
        if self._state == 'spline' and self.net_ui_context.hovered_near[0] == 'EDGE':
            ...
            p0 = loc3d_reg2D(context.region, context.space_data.region_3d, seg.n0.world_loc)
            p1 = loc3d_reg2D(context.region, context.space_data.region_3d, seg.n1.world_loc)
            if self.net_ui_context.hovered_mesh != {}:
                pn3d = self.net_ui_context.hovered_mesh['world loc']
                pn = loc3d_reg2D(context.region, context.space_data.region_3d, pn3d)
            else:
                pn = mouse_loc
            
            draw2d_polyline([p0,pn,p1], preview_line_clr, preview_line_wdth)
        ...
        if self._state != 'segmentation':
            ...
            for seg in self.spline_net.segments:
                ...
                if seg.is_inet_dirty:
                    ...
                    #TODO, check active target and change transparency
                    #if self.net_ui_context.target_dict[seg.points[0]] == 'MAX':
                    #    draw3d_polyline(seg.draw_tessellation, orange2, 4, view_loc, view_ortho)
                    #elif self.net_ui_context.target_dict[seg.points[0]] == 'MAND':
                    #    draw3d_polyline(seg.draw_tessellation, purple, 4, view_loc, view_ortho)
