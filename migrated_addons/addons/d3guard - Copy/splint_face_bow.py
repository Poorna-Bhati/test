...
def img_editor_draw_callback_px2(self, context):
    
    # draw text
    if len(self.pixel_coords) == 0:
        draw_typo_2d((1.0, .2, 1.0, 1), "Click First Point")
    elif len(self.pixel_coords) == 1:
        draw_typo_2d((1.0, .2, 1.0, 1), "Click Second Point")
    elif len(self.pixel_coords) == 2:
        draw_typo_2d((1.0, .2, 1.0, 1), "Press M to build Matrix")    
...
class D3Splint_OT_image_calibration(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        #add image empty
        empty = bpy.data.objects.new('Refernce Image',None)
        empty.empty_draw_type = 'IMAGE'
        img = self.imgeditor_area.spaces.active.image
        context.scene.objects.link(empty)
        empty.data = img
