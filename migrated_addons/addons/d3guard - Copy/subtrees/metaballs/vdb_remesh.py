...
# Blender 4.4: pyopenvdb replaced by openvdb
import openvdb
...
def convert_vdb(verts, tris, quads, vxsize):
    vtransform = vdb.createLinearTransform(voxelSize=vxsize)

    if len(tris) == 0 and len(quads) == 0:
        grid = vdb.FloatGrid.createLevelSetFromPoints(verts, transform=vtransform, radius=vxsize*2) 
    elif len(tris) == 0:
