...
from .bmesh_utils.bmesh_utilities_common import bmesh_loose_parts, new_bmesh_from_bmelements
from mathutils.bvhtree import BVHTree



def mask_to_vgroup(context, ob, group_name = '', action = 'REPLACE'):
    '''
    object should be active and in sculpt mode
    
    '''
    if action not in {'REPLACE', 'ADD', 'SUBTRACT'}:
        action = 'REPLACE'
    ...
    if group_name == '':
        vgroup = ob.vertex_groups.new('Mask G')
    else:
        vgroup = ob.vertex_groups.get(group_name)
        if not vgroup:
            vgroup = ob.vertex_groups.new(group_name)
    
    vgroup.add(mask_inds, 1, type = action)
    ...
    if context.mode == 'SCULPT' and ob.use_dynamic_topology_sculpting != is_dyntopo:
...
def find_inner_outer_shell(bme, bvh = None, delete_small = False, min_count = 100, test_size = 50, epsilon = .0001):
    ...
    islands = bmesh_loose_parts(bme, item_type = 'faces', selected = None, max_iters = 5000)
    if len(islands) == 0:
        return set(), set()
    if len(islands) == 1:
