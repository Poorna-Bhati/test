...
from ...bmesh_utils.bmesh_utilities_common import bound_box_bmverts, bbox_center
...
from .livecurves_datastructure import InputNetwork, InputPoint, InputSegment, SplineSegment, CurveNode, BezierBar
...
class Livecurves_UI_Tools():
    ...
    class NetworkUIContext():
        ...
        * Intermediary between polytrim_states and Network
        ...
        def __init__(self, context, model):
            ...
            self.max_bvh = BVHTree.FromObject(model, context.scene)
    ...
    def cache_to_bmesh(self):
        ...
        if 'Die Margin' not in bpy.data.objects:
            margin_me = bpy.data.meshes.new('Die Margin')
            margin_ob = bpy.data.objects.new('Die Margin', margin_me)
            bpy.context.scene.objects.link(margin_ob)
            
            edge_point_group = margin_ob.vertex_groups.new('edgpoints')
            
        else:
            margin_ob = bpy.data.objects.get('Die Margin')
    ...
    def save_splinenet_to_curves(self):
        ip_cycles, seg_cycles = self.spline_net.find_network_cycles()
        ...
        for ip_cycle in ip_cycles:
            crv_data = bpy.data.curves.new("Die Curve", "CURVE")
            crv_data.dimensions = '3D'
            crv_ob = bpy.data.objects.new("Die Curve", crv_data)
            bpy.context.scene.objects.link(crv_ob)
            crv_ob.parent = self.net_ui_context.max_model
            crv_ob.matrix_world = self.net_ui_context.max_model.matrix_world
        
            
        
            spline = crv_data.splines.new('BEZIER')
            spline.bezier_points.add(count = len(ip_cycle) - 1)
            imx_max = crv_ob.matrix_world.inverted()
            for i, node in enumerate(ip_cycle):
            ...
            me = crv_ob.to_mesh(self.context.scene, apply_modifiers = True, settings = 'PREVIEW')
            crv_ob = bpy.data.objects.new("Die Curve Mesh", me)
            bpy.context.scene.objects.link(crv_ob)
            crv_ob.parent = self.net_ui_context.max_model
            crv_ob.matrix_world = self.net_ui_context.max_model.matrix_world
            
            self.die_curves.append(crv_ob)
            
    
    def find_closest_die(self, loc):
        
        if len(self.expected_dies) == 0:
            return None
        return min(self.expected_dies, key = lambda x: (x.matrix_world.to_translation() - loc).length)
    
    def convert_curves_to_dies(self):
        
        
        for ob in self.die_curves:
            #mod = ob.modifiers.new('Shrinkwrap')
            bme = bmesh.new()
            bme.from_object(ob, self.context.scene)
            ...
            loc = bbox_center(bounds)
            
            
            empty_ob = self.find_closest_die(loc)
            if empty_ob:
                child = empty_ob.children[0]
            
                quat = child.matrix_world.to_quaternion()
            
                Z_prep = quat * Vector((0,0,-1))
                Z_die = quat * Vector((0,0,-1))
            else:
                Z_prep, Z_die = Vector((0,0,-1)), Vector((0,0,-1))
                
            angle = 4 * math.pi/180
            
            verts_loop = [v for v in bme.verts]
            
            #flatten
            vmax = max(verts_loop, key = lambda x: x.co.dot(Z_die))
            vmin = min(verts_loop, key = lambda x: x.co.dot(Z_die))
            max_co = vmax.co
            min_co = vmin.co
            
            original_eds = [ed for ed in bme.edges]
            ...
            for v in new_verts:
                v.co  = v.co + ((max_co - v.co).dot(Z_prep) + 20) * Z_prep 
            
            bme.faces.new(new_verts) 
                
            
            gdict = bmesh.ops.extrude_edge_only(bme, edges = original_eds)
            new_verts = set([ele for ele in gdict['geom'] if isinstance(ele, bmesh.types.BMVert)])
            new_eds = [ed for ed in bme.edges if all([v in new_verts for v in ed.verts[:]])]
            for v in new_verts:
                v.co  = v.co + ((min_co - v.co).dot(Z_die) + -2) * Z_die
                
                
            gdict = bmesh.ops.extrude_edge_only(bme, edges = new_eds)
            new_verts = [ele for ele in gdict['geom'] if isinstance(ele, bmesh.types.BMVert)]
            new_eds = [ed for ed in bme.edges if all([v in new_verts for v in ed.verts[:]])]
            bbox = bound_box_bmverts(new_verts)
            center = bbox_center(bbox)
            for v in new_verts:
                v.co = v.co + .1 * (center - v.co) - 6 * Z_die
            ...
            mod.object = self.net_ui_context.max_model
            mod.solver = 'CARVE'
