...
from .holemargin_datastructure import InputNetwork, NetworkCutter, SplineNetwork, InputPoint, InputSegment, SplineSegment, CurveNode
...
from ....operators_wizard.custom_tray.retention_holes import even_holes
class Polytrim_UI_Tools():
    ...
    def regrow_regions(self):
        if self._state != 'seed': return 
        
        self.seed_iterations += 5000
        for fp in self.network_cutter.face_patches:
            
            fp.grow_seed_faces(self.net_ui_context.bme, self.network_cutter.boundary_faces, self.seed_iterations)
            fp.color_patch()
    ...
    def cache_to_bmesh(self):
        ...
        if 'Hole Margin' not in bpy.data.objects:
            margin_me = bpy.data.meshes.new('Hole Margin')
            margin_ob = bpy.data.objects.new('Hole Margin', margin_me)
            bpy.context.scene.objects.link(margin_ob)
            
            edge_point_group = margin_ob.vertex_groups.new('edgpoints')
            
        else:
            margin_ob = bpy.data.objects.get('Hole Margin')
    ...
    def preview_holes(self):
        ...
        fs = p0.patch_faces.copy()
        
        
        if len(self.network_cutter.face_patches) > 1:
            for p in self.network_cutter.face_patches:
                if p == p0: continue
                fs.update(p.patch_faces)
        
        #create a new bmesh from the patch
        bme_patch = new_bmesh_from_bmelements(fs)
        
        
    def make_patch_plates(self):
        ...
        for p in self.network_cutter.face_patches:
            fs = p.patch_faces.copy()
            
            for seg in p.spline_net_segments:
                for iseg in seg.input_segments:
                    print('There are %i faces in face_chain')
                    fs.difference_update(iseg.face_chain)  
                
            #create a new bmesh from the patch
            bme_patch = new_bmesh_from_bmelements(fs)
            patch_data = bpy.data.meshes.new('Plate Patch')
            patch_obj = bpy.data.objects.new('Plate Patch', patch_data)
            
            self.context.scene.objects.link(patch_obj)
            
            bme_patch.to_mesh(patch_obj.data) 
            bme_patch.free()
        
        ###
        #bme_round_holes = even_holes(self.context, patch_obj, self.spacing, self.diameter)
        
        #if 'Retention Holes' not in bpy.data.objects:
        #    me = bpy.data.meshes.new('Retention Holes')
        #    ob = bpy.data.objects.new('Retention Holes', me)
        #    bpy.context.scene.objects.link(ob)
        #    ob.matrix_world = self.context.object.matrix_world
            
        #    mod = ob.modifiers.new('Solidify', type = 'SOLIDIFY')
        #    mod.offset = 0
        #    mod.thickness = 5.0
        #else:
        #    ob = bpy.data.objects.get('Retention Holes')
        #    me = ob.data
        
        
        
        #if 'Subtract Holes' not in self.context.object.modifiers:
        #    mod = self.context.object.modifiers.new('Subtract Holes', type = 'BOOLEAN')
        #    mod.object = ob
        ...
        #bme_round_holes.to_mesh(me)
        #bme_round_holes.free()
    ...
    def end_commit(self):
        ...
        #create the hole patch    
        if 'Hole Patch' in bpy.data.objects:
            patch_obj = bpy.data.objects.get('Hole Patch')
            patch_data = patch_obj.data
        else:
            
            patch_data = bpy.data.meshes.new('Hole Patch')
            patch_obj = bpy.data.objects.new('Hole Patch', patch_data)
            bpy.context.scene.objects.link(patch_obj)
            patch_obj.parent = self.context.object
        ...
        #prefs = get_settings()
        #print(prefs.auto_use_background)
        #if prefs.auto_use_background:
        #    print('starting the background director')
        #    bpy.ops.d3splint.bg_job_director()
            
        #    if splint.workflow_type not in {'SIMPLE_SHELL', 'DEPROGRAMMER', 'FREEFORM', 'BITE_POSITIONER'}:
                
        #        bpy.ops.d3splint.mark_occlusal_curves('INVOKE_DEFAULT')
                #or invoke_region_win
        #    print('added job director not waiting right?')
            
        #self.net_ui_context.ob.hide = True  #hide the optiized model
