...
from ...bmesh_utils.bmesh_utilities_common import edge_loops_from_bmedges_old, ensure_lookup, new_bmesh_from_bmelements, grow_selection, flood_selection_by_verts, flood_selection_edge_loop
...
from ...addon_common.common.rays import get_view_ray_data, ray_cast, ray_cast_path, ray_cast_bvh
from ...addon_common.common.simplify import simplify_RDP

from .smargin_datastructure import InputNetwork, NetworkCutter, SplineNetwork, InputPoint, InputSegment, SplineSegment, CurveNode
...
class Polytrim_UI_Tools():
    ...
    class NetworkUIContext():
        ...
        * Intermediary between polytrim_states and Network
        ...
        def __init__(self, context, ui_type='DENSE_POLY', geometry_mode = 'DESTRUCTIVE'):
            ...
            if "patches" not in self.ob.data.materials:
                ...
                if len(self.ob.material_slots) > 1:
                    mat0 = self.ob.material_slots[0].material
                    n = len(self.ob.material_slots) -1
                    self.ob.material_slots[n].material = mat0
                self.ob.material_slots[0].material = mat
                
            else:
                if self.ob.material_slots[0].material.name != 'patches':
                    for i, slot in enumerate(self.ob.material_slots):
            ...
            #insert vertex colori into copy data
            if "patches" not in self.ob.data.vertex_colors:
                vcol = self.ob.data.vertex_colors.new(name = "patches")
            else:
                vcol = self.ob.data.vertex_colors.get("patches")
                
                
            print('setting the active vertex color')
            self.ob.data.vertex_colors.active = vcol
            for ind, v_color in enumerate(self.ob.data.vertex_colors):
                if v_color == vcol:
                    break
            self.ob.data.vertex_colors.active_index = ind
            self.ob.data.vertex_colors.active = vcol
            vcol.active_render = True
            #print(vcol.name)
            #print(self.ob.data.vertex_colors.active.name)
        ...
        def update_bvh(self):
            '''
            update the bvh after self.bme has been modified by cutting
            '''
            start = time.time()
            del self.bvh
            self.bvh = BVHTree.FromBMesh(self.bme)
    ...
    def split_bad_spline_segments(self):
        ...
        for seg in bad_segs:
            ...
            if loc:
                ...
                node_mid = self.spline_net.create_point(mx * loc, loc, mid_view, idx)
    ...
    def connect_close_endpoints(self, threshold = 4.0): #todo provide option
        
        print('connect endpoints')
        endpoints = set(self.spline_net.get_endpoints())
        
        print('there are %i endpoints' % len(endpoints))
        new_segs = []
        connected_points = []
        itermax = len(endpoints)
        iters = 0
        while len(endpoints) > 1 and iters <= itermax:
            iters += 1
            #get any endpoint
            p0 = endpoints.pop()
            #find the closest one
            closest_endpoint = min(endpoints, key = lambda x: (p0.world_loc - x.world_loc).length)
            
            #check if close enough to connect
            R = (closest_endpoint.world_loc - p0.world_loc).length
            
            print('The endpoints are %f distance apart' % R)
            if R < threshold:
                print('connecting these endpoints')
                seg = self.spline_net.connect_points(p0, closest_endpoint)
                endpoints.remove(closest_endpoint)
                connected_points += [p0, closest_endpoint]
                new_segs += [seg]
    ...
    def find_calc_orphans(self):
        
        self.network_cutter.find_orphans()
        return
    
    
    def nudge_bad_segments(self, delta = .1): #todo provide option
        
        bad_segs = [seg for seg in self.input_net.segments if seg.bad_segment]
        
        seen_ips = set()
        for seg in bad_segs:
            seg.needs_calculation = True
            seg.calculation_complete = False
            dv = Vector((random.random(), random.random(), random.random()))
            
            for ip in seg.points:
                if ip in seen_ips:continue
                dv_n = dv - dv.dot(ip.view) * ip.view
                dv_n.normalize()
                shift = ip.local_loc + delta * dv_n
                loc, no, idx, d = self.net_ui_context.bvh.find_nearest(shift)
                
                if loc:
                    ip.face_ind = idx
                    ip.bmface = self.net_ui_context.bme.faces[idx]
                    ip.local_loc = loc
                    ip.world_loc = self.net_ui_context.mx * loc 
                    for lseg in ip.link_segments:
                        lseg.needs_calculation = True
                        lseg.caculation_complete = False
                        
        #we updated segments directly                            
        self.network_cutter.update_segments_async()
        self.network_cutter.validate_cdata()
    ...
    def seed_to_spline(self):  
        '''
        used to go backward from seed to boundary
        '''
    ...
    def segmentation_to_spline(self):
        '''
        used to clear data, remake bme, and go back to seed mode
        '''
        
        me_copy = self.net_ui_context.backup_data.copy()
        
        me_old = self.net_ui_context.ob.data
        self.net_ui_context.ob.data = me_copy
        ...
        if "patches" not in self.net_ui_context.ob.data.materials:
            self.net_ui_context.ob.data.materials.append(mat)
            if len(self.net_ui_context.ob.material_slots) > 1:
                mat0 = self.net_ui_context.ob.material_slots[0].material
                n = len(self.net_ui_context.ob.material_slots) -1
                self.net_ui_context.ob.material_slots[n].material = mat0
            self.net_ui_context.ob.material_slots[0].material = mat
            
        else:
            if self.net_ui_context.ob.material_slots[0].material.name != 'patches':
                for i, slot in enumerate(self.ob.material_slots):
                    if slot.material == mat:
                        n= i
                        break
        ...
        #insert vertex colori into copy data
        if "patches" not in self.net_ui_context.ob.data.vertex_colors:
            vcol = self.net_ui_context.ob.data.vertex_colors.new(name = "patches")
        else:
            vcol = self.net_ui_context.ob.data.vertex_colors.get("patches")
            
            
        print('setting the active vertex color')
        self.net_ui_context.ob.data.vertex_colors.active = vcol
        for ind, v_color in enumerate(self.net_ui_context.ob.data.vertex_colors):
            if v_color == vcol:
                break
        self.net_ui_context.ob.data.vertex_colors.active_index = ind
        self.net_ui_context.ob.data.vertex_colors.active = vcol
        vcol.active_render = True
        
        
        bme_replace = bmesh.new()
        bme_replace.from_mesh(self.net_ui_context.backup_data)
        ensure_lookup(bme_replace)
        self.net_ui_context.bme.free()
        self.net_ui_context.bme = bme_replace
        self.net_ui_context.non_man_bmverts = []
        self.net_ui_context.find_non_man()
        self.net_ui_context.non_man_eds = [ed.index for ed in self.net_ui_context.bme.edges if not ed.is_manifold]
        self.net_ui_context.non_man_ed_loops = edge_loops_from_bmedges_old(self.net_ui_context.bme, self.net_ui_context.non_man_eds)
            
        #clear out the network cutter
        del self.input_net
        del self.spline_net
    ...
    def cache_to_bmesh(self):
        ...
        if 'Tray Margin' not in bpy.data.objects:
            margin_me = bpy.data.meshes.new('Tray Margin')
            margin_ob = bpy.data.objects.new('Tray Margin', margin_me)
            bpy.context.scene.objects.link(margin_ob)
            
            edge_point_group = margin_ob.vertex_groups.new('edgpoints')
            
        else:
            margin_ob = bpy.data.objects.get('Tray Margin')
    ...
    def load_from_bmesh(self):
        '''
        No way to handle edgepoints yet, custom data layer?
        '''
        ...
        if 'Tray Margin' not in bpy.data.objects:
            ...
            margin_ob = bpy.data.objects.get('Tray Margin')
            margin_me = margin_ob.data
            vg = margin_ob.vertex_groups.get('edgepoints')
            
        bme_cache = bmesh.new()
        bme_cache.verts.ensure_lookup_table()
        bme_cache.edges.ensure_lookup_table()
          
        bme_cache.from_mesh(margin_me)
        bmv_to_node = {}
        
        #assumes only one vertex group
        edgepoint_inds = [v.index for v in margin_ob.data.vertices if v.groups]
        
        for bmv in bme_cache.verts:
            
            world_loc = bmv.co
            local_loc = self.net_ui_context.imx * world_loc
            ...
            node = self.spline_net.create_point(snap_wrld_loc, snap_loc, bmv.normal, ind)
            if bmv.index in edgepoint_inds:
                def intersect_d(ed):
                    inter, d = intersect_point_line(snap_loc, ed.verts[0].co, ed.verts[1].co)
    ...
    def end_commit(self):
        ...
        if self.net_ui_context.geometry_mode == 'DESTRUCTIVE':
            ...
            for i, mat in reversed(list(enumerate(self.net_ui_context.ob.data.materials))):
        ...
        #combine all patches into one patch
        p0 = self.network_cutter.face_patches[0]
        
        if len(self.network_cutter.face_patches) > 1:
            for p in self.network_cutter.face_patches:
                if p == p0: continue
                p0.patch_faces.update(p.patch_faces)
        
        
        #find and extend the perimeter for the perimeter strip
        boundary_edges = p0.find_all_boundary_edges()
        perim_faces =  set()
        for ed in boundary_edges:
            perim_faces.update([f for f in ed.link_faces])   
        
        bme_tray = new_bmesh_from_bmelements(p0.patch_faces)
        
        #perim_strip = grow_selection(self.net_ui_context.bme, perim_faces, max_iters = 5)
        #perim_strip.difference_update(p0.patch_faces)
         
        #create the trimmed model    
        if 'Tray Patch' in bpy.data.objects:
            trimmed_obj = bpy.data.objects.get('Tray Patch')
            trimmed_model = trimmed_obj.data
        else:
            
            trimmed_model = bpy.data.meshes.new('Tray Patch')
            trimmed_obj = bpy.data.objects.new('Tray Patch', trimmed_model)
            bpy.context.scene.objects.link(trimmed_obj)
            trimmed_obj.hide_select = True
        
        mat = bpy.data.materials.get("Blue Material")
        if mat is None:
        # create material
            mat = bpy.data.materials.new(name="Blue Material")
            mat.diffuse_color = Color((.1,.1,1))
            mat.use_transparency = True
            mat.transparency_method = 'Z_TRANSPARENCY'
            mat.alpha = .4
        
        trimmed_model.materials.append(mat) 
        bme_tray.to_mesh(trimmed_model)
        trimmed_model.update()
        
        #keep this for later mesh resolution optimization
        total_ed_len = sum([ed.calc_length() for ed in bme_tray.edges])
        avg_ed_len = total_ed_len/len(bme_tray.edges)
        print('DECIMATE TIME')
        print(avg_ed_len)
        decimate_ratio =  avg_ed_len / .3
        print(decimate_ratio)
        bme_tray.free()
        
        self.tray.tray_outline_done = True
