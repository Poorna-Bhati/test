...
from ..bmesh_fns import grow_selection, new_bmesh_from_bmelements
...
class PolyLineKnife(object):
    ...
    def confirm_cut_to_mesh_no_ops(self):
        ...
        #get the perimeter faces
        #perim_strip = grow_selection(self.bme, self.face_chain, max_iters = 5)
        
        #create the perim model
        j = bpy.context.scene.odc_splint_index
        splint =bpy.context.scene.odc_splints[j]
        
        Model = bpy.data.objects[splint.model]
        
        perim_strip = grow_selection(self.bme, self.face_chain, max_iters = 5)
        perim_strip_bme = new_bmesh_from_bmelements(perim_strip)
        #perim_strip_bme = bmesh.new()
        ...
        if 'Perim Model' in bpy.data.objects:
            ...
            bpy.context.scene.objects.link(perim_ob)
        
            cons = perim_ob.constraints.new('COPY_TRANSFORMS')
            cons.target = bpy.data.objects.get(splint.model)
        
        perim_strip_bme.to_mesh(perim_me)
        perim_strip_bme.free()
    ...
    def split_geometry(self, context):
        ...
        #if not (self.split and self.face_seed): 
        #    print('self.splt nd self.face seed not true')
        #    return
        ...
        output_bme.to_mesh(trimmed_model)
