...
from segmentation.mark_curves.livecurves_datastructure import InputNetwork, InputPoint, InputSegment, SplineSegment, CurveNode, BezierBar

import bgl
import odcutils
from common_utilities import get_settings, showErrorMessage
from odcutils import calculate_plane
...
class Livecurves_UI_Tools():
    ...
    def toggle_shell_vis(self):
        shell = bpy.data.objects.get('Splint Shell')
        if not shell: return
        shell.hide = shell.hide == False
        shell.show_transparent = True
    
    def toggle_max_vis(self):
        self.max_model.hide = self.max_model.hide == False
        
    def toggle_mand_vis(self):
        self.mand_model.hide = self.mand_model.hide == False
    ...
    def set_visualization_end(self):
        ...
        self.max_model.show_transparent = False
        self.mand_model.show_transparent = False
        
        rim = bpy.data.objects.get("Wax Rim Preview")
        if rim:
            rim.show_transparent = False
            
        shells = [ob for ob in bpy.data.objects if "Shell" in ob.name]
        for Shell in shells:
            Shell.hide = False
    ...
    def set_rim_alpha(self):
        alpha = self.rim_alpha
        mat1 = bpy.data.materials.get("Rim Material")
    ...
    def get_plane_object(self):
        if 'Inter Arch Surface' not in bpy.data.objects:
            ...
            # create material
                ...
                mat.diffuse_color = Color((.7, .2, .7))
                mat.use_transparency = True
                mat.transparency_method = 'Z_TRANSPARENCY'
                mat.alpha = .4
        
            new_ob.data.materials.append(mat)
            print('adding the material')
            ...
            if 'Solidify' not in new_ob.modifiers:
                mod = new_ob.modifiers.new('Solidify', type = 'SOLIDIFY')
    ...
    def get_rim_object(self):
        if 'Wax Rim Preview' not in bpy.data.objects:
            me = bpy.data.meshes.new('Wax Rim Preview')
            new_ob = bpy.data.objects.new('Wax Rim Preview', me)
            bpy.context.scene.objects.link(new_ob)
            new_ob.show_transparent = True
            ...
            new_ob = bpy.data.objects.get('Wax Rim Preview')
            new_ob.hide = False
            new_ob.show_transparent = True
            
            if 'Bevel' not in new_ob.modifiers:
                bmod = new_ob.modifiers.new('Bevel', type = 'BEVEL')
                bmod.offset_type = 'WIDTH'
                bmod.limit_method = 'ANGLE'
                bmod.width = 1.0
    ...
    def max_mand_to_plane_paths(self, Z, max_path, mand_path):
        #later, these will be spit out into new curves in a live bar object
        centroid, tangents, perps  = [], [], []
        ...
        #    tangents.reverse()
        #    tangents = [-1 * t for t in tangents]
        #    perps = [-1 * p for p in perps]
                
        return centroid, tangents, perps
    
        
    def preview_rim(self, spline_res = 80):
        return #or now
        #if not self.live_rim_preview:
        #    ob = bpy.data.objects.get('Wax Rim Preview')
        #    if ob:
        #        ob.hide = True
        #    return
    ...
    def preview_curved_plane(self, spline_res = 80, mode = 'CURVED'):
        #or now
        #if not self.live_rim_preview:
        #    ob = bpy.data.objects.get('Wax Rim Preview')
        #    if ob:
        ...
        new_ob = self.get_plane_object()
        bme = bmesh.new()
        ip_cycles, seg_cycles = self.spline_net.find_network_open_cycles()
        
        if len(ip_cycles) != 2:
            showErrorMessage('Cannot calculate plane for  segmental curves yet!.')
            return
        ...
        Z = Vector((0,0,1))  #todo, possibly best fit plane?  #yes
        Zfit = calculate_plane(max_path + mand_path)
        if Zfit.dot(Z) < 0:
            Zfit *= -1
        
        centroids, tangents, perps = self.max_mand_to_plane_paths(Zfit, max_path, mand_path)
        
        
        com = Vector((0,0,0))
        for loc in centroids:
            com = com  + 1/len(centroids) * loc
        
        
        bvs0 = []
        bvs1 = []
        for c, t, p in zip(centroids, tangents, perps):
            
            bvs0 += [bme.verts.new(c + 10 * p)]
            bvs1 += [bme.verts.new(c - 10 * p)]
            
            
        
            
            if len(bvs0) >= 2:
                bme.faces.new((bvs0[-1],bvs1[-1], bvs1[-2], bvs0[-2]))
        
        bvs0[0].co += - 5 * tangents[0]
        bvs0[-1].co += 5 * tangents[-1]
        bvs1[0].co += -5 * tangents[0]
        bvs1[-1].co += 5 * tangents[-1]
        
        
        if mode == 'FLAT':
            for v in bme.verts:
                r = v.co - com
                v.co = v.co - r.dot(Zfit)*Zfit
        
        #g_dict =bmesh.ops.extrude_face_region(bme,geom =  bme.faces[:])
        #new_vs = [ele for ele in g_dict['geom'] if isinstance(ele, bmesh.types.BMVert)]
        #for v in new_vs:
        #    v.co +=  .2 * v.normal
            
        
        new_ob.matrix_world = Matrix.Translation(com)
        bme.transform(new_ob.matrix_world.inverted())
    ...
    def preview_rim_HQ(self):
        
        self.fuse_rim_deactivate()
        
        #first, save the spline net to curves
        self.save_splinenet_to_curves()
        
        bpy.ops.d3splint.splint_rim_from_dual_curves(anterior_projection = self.anterior_projection,
                                                    flare = self.rim_flare,
                                                    anterior_segment = self.rim_ap_spread,
                                                    thickness_offset = self.thickness_offset,
                                                    ap_segment = self.ap_segment,
                                                    anterior_shift = self.anterior_shift)
        
        rim = bpy.data.objects.get('Wax Rim')
        rim.show_transparent = True
        
        #hide the rim preview:
        rim_pre = bpy.data.objects.get('Wax Rim Preview')
        if rim_pre:
            rim_pre.hide = True
    
    def fuse_rim_activate(self):
        
        Rim = bpy.data.objects.get('Wax Rim')
        if not Rim:
            showErrorMessage('Need to calculate the rim first!')
            return
        
        Shell = bpy.data.objects.get('Splint Shell')
        if not Shell:
            showErrorMessage('Splint Shell is not finished calculating!')
            return

        if 'Join Rim' in Shell.modifiers or self.splint.wax_rim_fuse == True:
            showErrorMessage('You have already fused it! If it appears that the rim was not successfully fused, please edit the rim slightly and fuse again', 100)
            return
    ...
    def volume_rim_activate(self):
        ...
        if self.splint.wax_rim_fuse == True:
            showErrorMessage('You have already fused it!', 100)
            return
            
        bpy.ops.d3splint.v_join_rim()
    ...
    def fuse_rim_deactivate(self):
        ...
        if bool_mod:  
            Shell.modifiers.remove(bool_mod)
        self.splint.wax_rim_fuse = False
        
        #not doing this anymore
        #else:   
        #    if 'splint_shell' in splint_cache.mesh_cache:
        #        bme_shell = splint_cache.mesh_cache['splint_shell']
        #        bme_shell.to_mesh(Shell.data)
        #    else:
        #        if 'shell backup' not in bpy.data.meshes:
        #            show_error_message('Cannot unfuse rim!')
        #            
        #            return
        #        me = bpy.data.meshes.get('shell backup')
        #        bme = bmesh.new()
        #        bme.from_mesh(me)
    ...
    def save_splinenet_to_curves(self):
        ...
        for ip_cycle in ip_cycles:
            print(self.net_ui_context.target_dict[ip_cycle[0]])
            if self.net_ui_context.target_dict[ip_cycle[0]] == 'MAX':
                max_cycles.append(ip_cycle)
            elif self.net_ui_context.target_dict[ip_cycle[0]] == 'MAND':
                mand_cycles.append(ip_cycle)
    ...
    def splinenet_to_occlusal_plane(self):
        ...
        #max plane
        vs_mand = [node.world_loc for node in mand_path]
        vs_max = [node.world_loc for node in max_path]
        
        Maxilla = bpy.data.objects.get(self.splint.get_maxilla())
        Mandible = bpy.data.objects.get(self.splint.get_mandible())
        
        
        Zfit = odcutils.calculate_plane(vs_mand + vs_max, itermax = 500, debug = False)
        if Zfit.dot(Vector((0,0,1))) < 0:
            Zfit *= -1
        
        
        X = Vector((1,0,0))
        center_max = Vector((0,0,0))
        for v in vs_max:
            center_max += 1/len(vs_max) * v
            
        
        center_mand= Vector((0,0,0))
        for v in vs_mand:
            center_mand += 1/len(vs_mand) * v
        
        Y = Zfit.cross(X)
        Xfit = Y.cross(Zfit)
           
        
        Rmax = r_matrix_from_principal_axes(Xfit, Y, Zfit).to_4x4()
        Rmand = r_matrix_from_principal_axes(Xfit, -Y, -Zfit).to_4x4()
        
        Tmax = Matrix.Translation(center_mand - 4 * Zfit)
        Tmand = Matrix.Translation(center_max + 4 * Zfit)
        ...
        if 'Max Occlusal Surface' not in bpy.data.objects:
            
            me = bpy.data.meshes.new('Max Occlusal Surface')
            bme.to_mesh(me)
            max_plane_obj = bpy.data.objects.new('Max Occlusal Surface', me)
            max_plane_obj.matrix_world = Tmax * Rmax
            ...
            max_plane_obj.data.materials.append(mat)
            bpy.context.scene.objects.link(max_plane_obj)
            max_plane_obj.hide = True
        else:
            max_plane_obj = bpy.data.objects.get('Max Occlusal Surface')
            max_plane_obj.matrix_world = Tmax * Rmax
            
        
        
        if 'Mand Occlusal Surface' not in bpy.data.objects:
            
            me = bpy.data.meshes.new('Mand Occlusal Surface')
            bme.to_mesh(me)
            mand_plane_obj = bpy.data.objects.new('Mand Occlusal Surface', me)
            mand_plane_obj.matrix_world = Tmand * Rmand
            ...
            mand_plane_obj.data.materials.append(mat)
            bpy.context.scene.objects.link(mand_plane_obj)
            mand_plane_obj.hide = True
        else:
            mand_plane_obj = bpy.data.objects.get('Mand Occlusal Surface')
            mand_plane_obj.matrix_world = Tmand * Rmand
        
        if Maxilla != None:
            for cons in max_plane_obj.constraints:
                    if cons.type == 'CHILD_OF':
                        max_plane_obj.constraints.remove(cons)
            
            cons = max_plane_obj.constraints.new('CHILD_OF')
            cons.target = Maxilla
            cons.inverse_matrix = Maxilla.matrix_world.inverted()
            
        
        if Mandible != None:
            for cons in mand_plane_obj.constraints:
                    if cons.type == 'CHILD_OF':
                        mand_plane_obj.constraints.remove(cons)
            
            cons = mand_plane_obj.constraints.new('CHILD_OF')
            cons.target = Mandible
            cons.inverse_matrix = Mandible.matrix_world.inverted()
    ...
    def end_commit(self):
        ...
        #calc occlusal plane/create plane
        self.splinenet_to_occlusal_plane()
        
        #mark appropriate properties in project datastructure
        self.splint.arch_curves_complete = True
        self.splint.ops_string += "Mark Occlusal Curves:"
        
        #delete wax preview
        if 'Wax Rim Preview' in bpy.data.objects:
            ob = bpy.data.objects.get('Wax Rim Preview')
            me = ob.data
        ...
        if 'Wax Rim' in bpy.data.objects:
            ...
            ob.hide = False
                ...
                #if we fused it...then do these things
                #if prefs.auto_blockout:
                #    bpy.ops.d3splint.metav_blockout_shell()
        
                #if prefs.auto_remesh_smooth:
                #    bpy.ops.d3splint.remesh_smooth_inflate()
