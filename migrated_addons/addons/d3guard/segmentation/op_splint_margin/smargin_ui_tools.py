...
class Polytrim_UI_Tools():
        ...
        if self.input_net.segments:
            ...
            if a and b:  #if points are not on the screen, a or b will be None
                ...
                if (dist < select_radius**2) and (bound < 1) and (bound > 0):
                    ...
                    if n0.view.dot(view_vector) < 0 and n1.view.dot(view_vector) < 0: #this is likely a segment on other side of mesh
                        return
    ...
    def cache_to_bmesh(self):
        ...
        if self.load_ob_name not in bpy.data.objects:
            margin_me = bpy.data.meshes.new(self.load_ob_name)
            margin_ob = bpy.data.objects.new(self.load_ob_name, margin_me)
            bpy.context.scene.objects.link(margin_ob)
            
            edge_point_group = margin_ob.vertex_groups.new('edgpoints')
            
        else:
            margin_ob = bpy.data.objects.get(self.load_ob_name)
    ...
    def end_commit(self):
        ...
        raw_model = bpy.data.objects.get(self.model_name)
        ...
        #self.out_model_ame = 'Shell Patch'
        #create the trimmed model    
        if self.out_model_name in bpy.data.objects:
            patch_obj = bpy.data.objects.get(self.out_model_name)
            
        else:
            
            patch_data = bpy.data.meshes.new(self.out_model_name)
            patch_obj = bpy.data.objects.new(self.out_model_name, patch_data)
        ...
        setattr(splint, self.done_prop, True)
        splint.ops_string += self.ops_record
