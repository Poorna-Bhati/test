...
class Polytrim_States():
    ...
    def seed_can_enter(self):
        ...
        if any([ip.is_endpoint for ip in self.input_net.points]):
            self.hint_bad = True
            show_error_message('There are breaks in the margin!  Look for yellow dots and adjust the margin or use the "Clonnect Endpoints" tool.')
            return False
        return len(ip_cyc) > 0
    ...
    def segmentation_can_enter(self):
        ...
        return len(self.network_cutter.face_patches) > 0
    ...
    def region_enter(self):
        ...
        self.network_cutter.find_boundary_faces_cycles()
        for patch in self.network_cutter.face_patches:
            patch.grow_seed_faces(self.input_net.bme, self.network_cutter.boundary_faces, max_iters = self.seed_iterations)
            patch.color_patch()
    ...
    def spline_main(self):
        ...
        if self.actions.timer:
            
            #make sure we aren't checking constantly
            dt = time.time() - self.last_bad_check
            if dt < 1.5: return
            
            self.last_bad_check = time.time()  #doing this here means it has to wait until 1 sec after all calculations complete
            
            #don't keep recursively dividing segments
            if self.check_depth > 3: 
    ...
    def spline_add_point(self):
        ...
        self.last_bad_check = time.time() #prevent trying to correct errors while adding points
        assert s and n
        self.add_spline(s, n)
        self.net_ui_context.selected = n
