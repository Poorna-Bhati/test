...
class Polytrim_UI_Tools():
    ...
    def end_commit(self):
        ...
        bme_patch = new_bmesh_from_bmelements(p0.patch_faces)
        bvh_patch = BVHTree.FromBMesh(bme_patch)
        
        
        
        bme_filter = bmesh.new()
        raw_model = bpy.data.objects.get(splint.model)
        bme_filter.from_mesh(raw_model.data)
        
        delete = set()
        for v in bme_filter.verts:
            loc, no, ind, d =  bvh_patch.find_nearest(v.co, 1.5)
            if not loc:
                delete.add(v)
              
        bmesh.ops.delete(bme_filter, geom = list(delete), context = 1)
        
        #create the trimmed model    
        if 'Refractory Scaffold' in bpy.data.objects:
            trimmed_obj = bpy.data.objects.get('Refractory Scaffold')
            trimmed_model = trimmed_obj.data
        else:
            
            trimmed_model = bpy.data.meshes.new('Refractory Scaffold')
            trimmed_obj = bpy.data.objects.new('Refractory Scaffold', trimmed_model)
            bpy.context.scene.objects.link(trimmed_obj)
            cons = trimmed_obj.constraints.new('COPY_TRANSFORMS')
            cons.target = raw_model
        
        
        bme_filter.to_mesh(trimmed_obj.data) 
        bme_filter.free()
        
        
        #create the trimmed model    
        if 'Shell Patch' in bpy.data.objects:
            patch_obj = bpy.data.objects.get('Shell Patch')
            patch_data = trimmed_obj.data
        else:
            
            patch_data = bpy.data.meshes.new('Shell Patch')
            patch_obj = bpy.data.objects.new('Shell Patch', patch_data)
            bpy.context.scene.objects.link(patch_obj)
            cons = patch_obj.constraints.new('COPY_TRANSFORMS')
            cons.target = raw_model
        ...
        bpy.context.scene.objects.active = trimmed_obj
        ...
        bpy.context.scene.tool_settings.sculpt.constant_detail_resolution = 5.5
