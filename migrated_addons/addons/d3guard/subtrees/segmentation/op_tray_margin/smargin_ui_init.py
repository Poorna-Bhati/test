...
from .smargin_datastructure import InputPoint, SplineSegment, CurveNode
...
class Polytrim_UI_Init():
    def ui_setup(self):
        ...
        def mode_change_update_menus():
            nonlocal win_obvious_instructions
            m = self._state
            if m == 'spline':
                win_obvious_instructions.hbf_title.set_label('Draw the Perimeter of the Tray')
            elif m == 'seed':
                win_obvious_instructions.hbf_title.set_label('Pick Interior Region of Tray')
            elif m == 'segmentation':
        ...
        win_obvious_instructions = self.wm.create_window('Draw the Perimeter of the Tray', {'pos':8, "vertical":False, "padding":15, 'movable':True, 'bgcolor':(0.50, 0.50, 0.50, 0.5), 'border_color':(0.50, 0.50, 0.50, 0.9), "border_width":4.0})
        ...
        #win_tools = self.wm.create_window('Splint Margin Tools', {'pos':1, 'movable':True, 'bgcolor':(0.50, 0.50, 0.50, 0.90)})
        ...
        info = self.wm.create_window('Membrane Margin Help', {'pos':9, 'movable':True, 'bgcolor':(0.50, 0.50, 0.50, 0.90)})
        ...
        find_problems = precut_options.add(ui.UI_Button('Show Problems', self.connect_close_endpoints, margin = 5))
