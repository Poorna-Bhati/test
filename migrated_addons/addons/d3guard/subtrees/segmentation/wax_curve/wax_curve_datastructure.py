...
from ...bmesh_utils.bmesh_utilities_common import grow_selection_to_find_face, flood_selection_faces, face_region_boundary_loops, edge_loops_from_bmedges_old, edge_loops_from_bmedges, flood_selection_by_verts, flood_selection_edge_loop, ensure_lookup
from ...bmesh_utils.bmesh_utilities_segmentation import  bmesh_loose_parts_faces
...
from ..common.maths import intersect_path_plane, space_evenly_on_path
...
class BMFacePatch(object):
    ...
    def grow_seed(self, bme, boundary_edges):
        ...
        if len(self.patch_faces)  != 0:
            self.un_color_patch()
        island = flood_selection_edge_loop(bme, boundary_edges, self.seed_face, max_iters = 30000)
...
class SplineNetwork(object): #InputNetwork
    ...
    def create_point(self, world_loc, local_loc, view, face_ind):
        ''' create an InputPoint '''
        self.points.append(CurveNode(world_loc, local_loc, view, face_ind))
        return self.points[-1]
    ...
    def find_network_open_cycles(self):  #TODO
        #this is the equivalent of "edge_loops"
        #TODO, mirror the get_cycle method from polystrips
        #right now ther eare no T or X junctions, only cuts across mesh or loops within mesh
        #will need to implement "IputNode.get_segment_to_right(InputSegment) to take care this
        
        print('OPEN SPLINE NETWORK CYCLES')
        ...
        return unclosed_ip_cycles, unclosed_seg_cycles


    
  
class BezierWax(object):
    def __init__(self, spline_network, obj):
        
        self.spline_net = spline_network  #<--may hold multiple bars?
        
        self.splines = []
        self.nodes = []
        
        self.obj = obj  #todo, re-label "mesh obj"  'tess_obj"
        self.bme = bmesh.new()
        self.bvh = None
    
    def from_paths_uniform_nodes(self, vert_paths, epsilon = 0.4):
        assert len(vert_paths) >= 3
        
        imx = self.obj.matrix_world.inverted()
        self.splines = []
        self.nodes = []
        
        #we pick the first path to generate fom
        p0 = vert_paths[0]
        ...
        def po_tan(ind):
            ...
            if ind == N-1:
                t_vect = p0[N-1] - p0[N-2]
            else:
                t_vect = p0[ind+1] - p0[ind - 1]
            
            t_vect.normalize()
            return t_vect
            
        feature_inds = simplify_RDP(p0, epsilon)
        new_points = []
        new_segs = []
        prev_pnt = None
        for ind in feature_inds:
            
            new_pnt = self.spline_net.create_point(p0[ind], imx * p0[ind], Vector((0,0,1)), -1)
            new_points += [new_pnt]
            if prev_pnt:
                seg = SplineSegment(prev_pnt,new_pnt)
        ...
        self.splines += [new_segs]
        self.nodes += [new_points]
            
        for vp in vert_paths[1:]:
            
            prev_pnt = self.spline_net.create_point(vp[0], imx * vp[0], Vector((0,0,1)), -1)
            
            new_points = [prev_pnt]
            new_segs = []
            for ind in feature_inds[1:len(feature_inds)-1]:
                p_inter = intersect_path_plane(vp, p0[ind], po_tan(ind))
                if not p_inter: continue                
                new_pnt = self.spline_net.create_point(p_inter, imx * p_inter, Vector((0,0,1)), -1)
                new_points += [new_pnt]
                if prev_pnt:
                    print(prev_pnt)
                    seg = SplineSegment(prev_pnt,new_pnt)
            ...
            new_pnt = self.spline_net.create_point(vp[-1], imx * vp[-1], Vector((0,0,1)), -1)
        ...
        for ips in self.nodes:
            print('There are %i nodes' % len(ips))
        for segs in self.splines:
            print('There are %i segs' % len(segs))
        return
    
    def from_wax_object(self, rim_obj):
        
        if len(rim_obj.children) == 0: return
        
        curves = [ob for ob in rim_obj.children if ob.type == 'CURVE']
        imx = self.obj.matrix_world.inverse()
        view = Vector((0,0,1))
        for c in curves:
            mx = c.matrix_world
            for spline in c.splines:
                ...
                self.splines += [new_segs]
                self.nodes += [new_pts]
        #check for child curves
        #convert them to spline net curves
        #teseelate them
        #rebuild tesselation
        
        return
    
    def cache_rim_object(self):
        
        children = [ob.name for ob in self.obj.children if ob.type == 'CURVE']
        
        if self.obj.name + "_spline" in children:
            spline_obj = bpy.data.objects.get(self.obj.name + "_spline")
            ...
            spline_data = bpy.data.curves.new(self.obj.name + "_spline", 'CURVE')
            crv_obj = bpy.data.objects.new(self.obj.name + "_spline", spline_data)
    ...
    def tessellate_cycles(self, cycle_data, tess_res):
        ...
        for ips, segs in cycle_data:
            raw_tess_cyc = []
            print('there are %i ips in this data' % len(ips))
            ...
            print("there are %i points in raw test cycle" % len(raw_tess_cyc))
            
            v_final, eds = space_evenly_on_path(raw_tess_cyc, [(0,1),(1,2)], tess_res)
            tess_cycles += [v_final]
        return tess_cycles
    
    def tessellate_bar(self, resolution = 80):
        #we assume the
        #tesslate each spline evenly
        #splines are always kept in order
        #
        cycle_data = zip(self.nodes, self.splines)
        tess_cycles = self.tessellate_cycles(cycle_data, resolution)
        
        bme = bmesh.new()
        #make the paths
        loft_paths = []
        for vp in tess_cycles:
            loft_paths.append([bme.verts.new(v) for v in vp])
        
        N = len(tess_cycles[0])
        bme.faces.new([p[0] for p in loft_paths[::-1]])
        bme.faces.new([p[N-1] for p in loft_paths])  
        
        for i in range(0, len(loft_paths)):
            p = loft_paths[i]
            if i == len(loft_paths) - 1:
                pm1 = loft_paths[0]
            else:
                pm1 = loft_paths[i + 1]
        
            for j in range(0, N-1):
                bme.faces.new((pm1[j], pm1[j+1], p[j+1], p[j])) 
             
        bme.to_mesh(self.obj.data)
        bme.free()
    ...
    def bevel_spline(self,spline, offset, offset_type = 'PERCENTAGE'):
        
        #find adjacent splines
        
        #duplicate spline 
        
        #insert it between above and below
