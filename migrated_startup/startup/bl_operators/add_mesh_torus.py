...
Replace the deprecated usage of EnumProperty as follows:

```python
import bpy

# Old (deprecated in Blender 4.4 and earlier):
# mode = bpy.props.EnumProperty(
#     name="Mode",
#     description="Choose mode",
#     items=[('A', "A", "Mode A"), ('B', "B", "Mode B")]
# )

# Blender 4.4+ compatible:
mode: bpy.props.EnumProperty(
    name="Mode",
    description="Choose mode",
    items=[
...
Key changes:
- Use **type annotations** (e.g., mode: bpy.props.EnumProperty(...)) instead of assignment (mode = ...).
- This is now required for all property definitions in Blender 2.8+ and remains the standard in Blender 4.4[1].
