...
Replace the deprecated usage of EnumProperty as follows for Blender 4.4 compatibility:
...
Key changes:
- Use **type annotations** (the colon syntax) instead of assignment for class properties.
- Ensure all required keyword arguments (`name`, `description`, `items`) are provided explicitly[4].
- The `items` argument must be a list of tuples or a callable returning such a list[4].

If you are defining this inside a class, it should look like:
...
This syntax is required for Blender 2.80+ and is fully compatible with Blender 4.4[4].
